...
Function EnsureChoiceField {
    ...
    param (
        ...
        [Parameter(Mandatory = $true, HelpMessage = "The choices for the field.")]
        [string[]]$Choices,

        [Parameter(Mandatory = $false, HelpMessage = "Flag to indicate if the field allows multiple choices.")]
    ...
    if ($existingField) {
        Write-Warning "Choice field with name $InternalName already exists!"
        return
    }
    ...
    $params = @{
        ...
        Type         = if ($AllowMultipleValues) { "MultiChoice" } else { "Choice" }
        Group        = $Group
        Required     = $Required
        Id           = $FieldId
        Choices      = $Choices
    ...
    Write-Host "Choice field $InternalName created!" -ForegroundColor Green
}
